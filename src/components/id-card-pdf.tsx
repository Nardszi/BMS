...
export function IDCardPDF({ data, showPrintLayout = false, captureId, forExport }: IDCardPDFProps) {
  ...
  const usePixels = forExport || !!captureId;

  const cardStyle: React.CSSProperties = usePixels
    ? { width: CR80_WIDTH_PX, height: CR80_HEIGHT_PX }
    : { width: "3.375in", height: "2.125in" };

  const fontSize = usePixels ? { header: 22, subheader: 14, name: 24, body: 16, small: 13, tiny: 11 }
    : { header: "7px", subheader: "5px", name: "8px", body: "6px", small: "5px", tiny: "5px" };

  const photoStyle: React.CSSProperties = usePixels
  ...
  const cardFront = (
    <div
      ...
      style={{ ...cardStyle, borderRadius: usePixels ? 12 : undefined }}
    >
      {/* Top Header */}
      <div className="flex items-center justify-center gap-2 bg-blue-900 px-2 py-1" style={usePixels ? { padding: "10px 16px" } : {}}>
        <img src="/barangay-seal.png" alt="" className="h-6 w-6 object-contain" style={usePixels ? { width: 48, height: 48 } : undefined} />
      ...
      {/* Main Content */}
      <div className="flex gap-2" style={usePixels ? { padding: "16px 20px 8px", gap: 16 } : { padding: "4px 8px 2px" }}>
        {/* Photo */}
        <div className="flex-shrink-0">
          {data.photoUrl ? (
            ...
            <div style={{ ...photoStyle, display: "flex", alignItems: "center", justifyContent: "center", border: "2px dashed #d1d5db", borderRadius: 4, backgroundColor: "#f9fafb", fontSize: usePixels ? 20 : 10, color: "#9ca3af" }}>
              No Photo
            </div>
          )}
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div style={{ marginTop: usePixels ? 6 : 2, display: "flex", flexDirection: "column", gap: usePixels ? 3 : 1 }}>
          ...
          <div style={{ marginTop: usePixels ? 8 : 3, backgroundColor: "#eff6ff", padding: usePixels ? "4px 8px" : "2px 4px", borderRadius: 4 }}>
      ...
      {/* Bottom */}
      <div className="absolute bottom-0 left-0 right-0 flex items-center justify-between border-t border-blue-100 bg-blue-50" style={usePixels ? { padding: "6px 16px" } : { padding: "2px 8px" }}>
  ...
  const cardBack = (
    ...
    >
      {/* Header */}
      <div className="bg-blue-900" style={usePixels ? { padding: "10px 16px" } : { padding: "4px 8px" }}>
        <p className="text-center" style={{ fontSize: fontSize.header, fontWeight: 700, color: "white" }}>BARANGAY IDENTIFICATION CARD</p>
      </div>

      {/* Back Content */}
      <div style={usePixels ? { padding: "16px 20px 8px" } : { padding: "6px 8px 2px" }}>
        <p style={{ fontSize: fontSize.small, fontWeight: 700, color: "#374151", marginBottom: usePixels ? 6 : 2 }}>EMERGENCY CONTACT</p>
        <div style={{ backgroundColor: "#f9fafb", padding: usePixels ? 10 : 4, borderRadius: 4, fontSize: fontSize.body, color: "#4b5563" }}>
          <p>In case of emergency, please contact:</p>
          <p style={{ fontWeight: 600 }}>{r.lastName} - {r.contactNumber || "N/A"}</p>
        </div>

        <div className="flex items-end justify-between" style={{ marginTop: usePixels ? 20 : 6 }}>
          <div className="text-center">
            <div style={usePixels ? { width: 80, height: 80, border: "1px solid #d1d5db", borderRadius: 4, backgroundColor: "#f9fafb", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 4 } : { width: "0.4in", height: "0.4in", border: "1px solid #d1d5db", borderRadius: 4, backgroundColor: "#f9fafb", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 2 }}>
              <img src="/barangay-seal.png" alt="" className="h-6 w-6 object-contain opacity-30" style={usePixels ? { width: 48, height: 48, opacity: 0.3 } : undefined} />
            </div>
            <p style={{ fontSize: fontSize.tiny, color: "#6b7280" }}>Barangay Seal</p>
          </div>

          <div className="text-center">
            <div style={usePixels ? { width: 200, height: 60, borderBottom: "1px solid #9ca3af", marginBottom: 4 } : { width: "1.2in", height: "0.4in", borderBottom: "1px solid #9ca3af", marginBottom: 2 }} />
            ...
            <div style={usePixels ? { width: 80, height: 80, border: "1px solid #d1d5db", borderRadius: 4, backgroundColor: "#f9fafb", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 4 } : { width: "0.4in", height: "0.4in", border: "1px solid #d1d5db", borderRadius: 4, backgroundColor: "#f9fafb", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 2 }}>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 1 }}>
                {Array.from({ length: 25 }).map((_, i) => (
                  <div key={i} style={{ width: usePixels ? 10 : 3, height: usePixels ? 10 : 3, backgroundColor: Math.random() > 0.5 ? "#000" : "transparent" }} />
                ))}
              </div>
            </div>
      ...
      {/* Footer */}
      <div className="absolute bottom-0 left-0 right-0 border-t border-blue-100 bg-blue-50" style={usePixels ? { padding: "6px 16px" } : { padding: "2px 8px" }}>
