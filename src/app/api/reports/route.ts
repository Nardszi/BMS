...
export async function GET(request: Request) {
  ...
  const purokDetails = purokData.map((p) => {
    ...
    return {
      ...
      population: pop,
      households: hhs,
      avgSize: hhs > 0 ? Math.round((pop / hhs) * 10) / 10 : 0,
    };
  });
  ...
  return NextResponse.json({
    ...
    sexRatio: ageGroups.Children + ageGroups.Adults + ageGroups.Seniors > 0
      ? (() => {
    ...
    permitStats: {
      ...
      active: permitMap["ACTIVE"] || 0,
      expired: permitMap["EXPIRED"] || 0,
      revoked: permitMap["REVOKED"] || 0,
    },
    blotterStats: {
      total: blotterStats.reduce((sum, b) => sum + b._count, 0),
      open: blotterMap["OPEN"] || 0,
      resolved: blotterMap["RESOLVED"] || 0,
      escalated: blotterMap["ESCALATED"] || 0,
    },
    monthlyResidentTrend,
    monthlyCertTrend,
    topPurok: sorted[0] || null,
    leastPurok: sorted[sorted.length - 1] || null,
  });
}
